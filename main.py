...
binary_threshold = 130 #設定二值化閾值是130
segmentation_spacing = 0.95 #分割間距
file_path = ''
newpath = ''
        ...
        while n < width - 1:
            ...
            if white[n] > ((1 - segmentation_spacing) * white_max):
                ...
                if end - start > 5:
                    ...
                    # cv2.waitKey(0)
                    # path = 'pic\\cut\\'
                    # print(str(imageInPath) + '_' + str(ap) + '.png')
                    cv2.imwrite(str(imageInPath) + '_' + str(ap) + '.png', cj)
        # cv2.imshow('test',img)
        pixMap = QPixmap(imageOutPath)#.scaled(self.label_OriginalPhoto.width(), self.label_OriginalPhoto.height())
        self.label_Binary.setPixmap(pixMap)
        text = pytesseract.image_to_string(img, lang='eng', config='--psm 7')  # psm 7在辨識時能把圖片當成一行字 增加辨識率
        print(text)
...
if __name__ == '__main__':
    #車牌定位
    app = PyQt5.QtWidgets.QApplication(sys.argv)  # 创建一个QApplication，也就是你要开发的软件app
    MainWindow = PyQt5.QtWidgets.QMainWindow()  # 创建一个QMainWindow，用来装载你需要的各种组件、控件
